...
public class NioSSLExample
{
   ...
   {
      InetSocketAddress address = new InetSocketAddress("s5381310.fibi.corp", 443);
      ...
      final NioSSLProvider ssl = new NioSSLProvider(key, engine, ioBufferSize, ioWorker, taskWorkers)
      {
         ...
         public void onSuccess()
         {
            ...
            http.append("GET /oded.html HTTP/1.0\r\n");
         ...
         {
            // HTTP response
        	 System.out.println ("==================== starting on input");
      ...
      while (true)
      {
         ...
         while (keys.hasNext())
         {
            keys.next();
            keys.remove();
            System.out.println("==================== perform ssl.processInput");
            ssl.processInput();
         }
      }
   }
