...
public class JDFrame extends JFrame implements ActionListener {
	...
	public JDFrame(JDCaptor captor) {
		...
		statMenu.add(menu);
		JDStatisticsTaker[] stakers = JDStatisticsTakerLoader
				.getStatisticsTakers();
		...
		menuBar.add(menu);
		item = createLaFMenuItem("Metal",
				"javax.swing.plaf.metal.MetalLookAndFeel");
		menu.add(item);
		item.setSelected(true);
		menu.add(createLaFMenuItem("Windows",
				"com.sun.java.swing.plaf.windows.WindowsLookAndFeel"));
		menu.add(createLaFMenuItem("Motif",
				"com.sun.java.swing.plaf.motif.MotifLookAndFeel"));
		menu.add(createLaFMenuItem("Mac",
	...
	public void actionPerformed(ActionEvent evt) {
		...
		if (cmd.equals("Open")) {
			...
			captor.addCumulativeStatFrame(JDStatisticsTakerLoader
					.getStatisticsTakerAt(index));
		} else if (cmd.startsWith("CONSTAT")) {
			int index = Integer.parseInt(cmd.substring(7));
			captor.addContinuousStatFrame(JDStatisticsTakerLoader
					.getStatisticsTakerAt(index));
	...
	/*
	 * void initInternalFrames(){ packets.removeAllElements();
	 * totalPacketCount=0; tableFrame.clear();
	 * 
	 * if(sframes!=null) for(int i=0;i<sframes.length;i++) if(sframes[i]!=null)
	 * sframes[i].clear(); }
	 */
	...
	javax.swing.Timer JDFrameUpdater = new javax.swing.Timer(500,
			new ActionListener() {
				public void actionPerformed(ActionEvent evt) {
					tablePane.fireTableChanged();
					statusLabel.setText("Captured "
							+ captor.getPackets().size() + " packets.");
					repaint();
				}
			});

	void loadProperty() {
		setSize(Integer.parseInt(JpcapDumper.JDProperty.getProperty("WinWidth",
				"400")), Integer.parseInt(JpcapDumper.JDProperty.getProperty(
				"WinHeight", "400")));
		setLocation(Integer.parseInt(JpcapDumper.JDProperty.getProperty("WinX",
				"0")), Integer.parseInt(JpcapDumper.JDProperty.getProperty(
				"WinY", "0")));
	}
	...
	void saveProperty() {
		...
		JpcapDumper.JDProperty.put("WinWidth", String
				.valueOf(getBounds().width));
		JpcapDumper.JDProperty.put("WinHeight", String
				.valueOf(getBounds().height));
