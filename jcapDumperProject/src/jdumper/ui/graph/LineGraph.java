...
public class LineGraph extends JPanel {
	...
	private Color[] colors = { Color.blue, Color.green, Color.yellow.darker(),
			...
			for (int i = 0; i < values.length; i++) {
				...
				if (autoMin || autoMax) {
					for (int j = 0; j < values[i].length; j++) {
						if (autoMax && values[i][j] > maxValue)
							maxValue = values[i][j];
						if (autoMin && values[i][j] < minValue)
							minValue = values[i][j];
					}
				}
			}
	...
	public void addValue(long[] values) {
		...
		if (autoMin || autoMax) {
			for (int i = 0; i < values.length; i++) {
				if (autoMax && values[i] > maxValue)
					maxValue = values[i];
				if (autoMin && values[i] < minValue)
					minValue = values[i];
			}
		}
		repaint();
	...
	private class GraphPane extends JPanel {
		public void paintComponent(Graphics g) {
			...
			if (labels == null || values == null || values.size() == 0)
				return;
			// calc font size
			int ylabelw = 0;
			for (int i = 0; i < 4; i++) {
				int w = g
						...
						.stringWidth(
								String
										.valueOf((double) (maxValue - (maxValue - minValue)
												* i / 4.0)));
				if (w > ylabelw)
					ylabelw = w;
			}
			long h = getHeight() - marginY - marginY, w = getWidth(), h2 = maxValue
					- minValue;
			double d = (double) (w - marginX - marginX) / (values.size() - 1.0), x = d
					+ marginX + ylabelw;
			// draw X/Y axis
			...
			for (int i = 0; i < 5; i++) {
				...
				g.drawString(String
						.valueOf((double) (maxValue - (maxValue - minValue) * i
								/ 4.0)), marginX - 5, y);
			}
			...
			for (int i = 1; i < values.size(); i++, x += d) {
				...
				for (int j = 0; j < v.length; j++) {
					...
					for (int k = 0; k < j / colors.length; k++)
						c.darker();
					g.setColor(c);
					// ((Graphics2D)g).setStroke(new BasicStroke(2.0f));
					// ((Graphics2D)g).draw(new
					// Line2D.Double((int)(x-d),h+marginY-(vv[j]-minValue)*h/h2,
					// (int)x,h+marginY-(v[j]-minValue)*h/h2));
					g.drawLine((int) (x - d),
							(int) (h + marginY - (vv[j] - minValue) * h / h2),
							(int) x, (int) (h + marginY - (v[j] - minValue) * h
									/ h2));
				}
				vv = v;
			}
	...
	private class LabelPane extends JPanel {
		LabelPane() {
			...
			setBorder(new CompoundBorder(BorderFactory.createLineBorder(
					Color.black, 1), new EmptyBorder(10, 10, 10, 10)));
		}
