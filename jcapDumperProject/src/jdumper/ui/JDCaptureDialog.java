...
public class JDCaptureDialog extends JDialog implements ActionListener {
	...
	public JDCaptureDialog(JFrame parent) {
		...
		caplenPane.setBorder(BorderFactory
				.createTitledBorder("Max capture length"));
			...
			try {
				...
				if (caplen < 68 || caplen > 1514) {
					JOptionPane.showMessageDialog(null,
							"Capture length must be between 68 and 1514.");
					return;
				}
				jpcap = JpcapCaptor.openDevice(devices[adapterComboBox
						.getSelectedIndex()], caplen,
						promiscCheck.isSelected(), 50);
				if (filterField.getText() != null
						&& filterField.getText().length() > 0) {
					jpcap.setFilter(filterField.getText(), true);
				}
			} catch (NumberFormatException e) {
				JOptionPane.showMessageDialog(null,
						"Please input valid integer in capture length.");
