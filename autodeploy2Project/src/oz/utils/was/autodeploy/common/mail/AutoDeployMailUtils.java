package oz.utils.was.autodeploy.common.mail;
...
public class AutoDeployMailUtils {
	...
	private final static String PASSWORD = "ppui78";
	...
	private static Properties getEmailProperties(final Properties emailProperties,
			final Properties emailMessagesProperties, final AutoDeployMailMessageEnum autoDeployMailMessageEnum,
	...
	private static void sendGmMail(final Properties emailMessagesProperties,
		...
		PropertiesUtils.printProperties(gmMailProperties, Level.FINEST);
		...
		Properties emailProperties = getEmailProperties(gmMailProperties, emailMessagesProperties,
				autoDeployMailMessageEnum, earFileParameters);
		GMUtils.sendEmail(emailProperties);
	}

	public static void sendMail(final Properties emailMessagesProperties,
			final AutoDeployMailMessageEnum autoDeployMailMessageEnum, final EarFileParameters earFileParameters) {
	...
	private static void sendSmtpMail(final Properties emailMessagesProperties,
			final AutoDeployMailMessageEnum autoDeployMailMessageEnum, final EarFileParameters earFileParameters) {
		Properties smtpEmailProperties = FibiUtils.getFbiMailProperties();
		PropertiesUtils.printProperties(smtpEmailProperties, Level.INFO);
		...
		Properties emailProperties = getEmailProperties(smtpEmailProperties, emailMessagesProperties,
				autoDeployMailMessageEnum, earFileParameters);
