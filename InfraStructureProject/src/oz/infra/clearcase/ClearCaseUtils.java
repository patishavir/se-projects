...
public class ClearCaseUtils {
	...
	public static enum VIEWTYPE_ENUM {
		SNAPSHOT, DYNAMIC, NOT_A_VIEW
	}
	...
	public static int cloneStream(final String pvob, final String stream2Clone, final String newStream,
			final String newStreamDispositionString) {
		...
		String delimiter = "!";
		String ccParamsString = StringUtils.concat(delimiter, delimiter, "mkstream", delimiter, "-in", delimiter,
				parentStream, "@", pvob, delimiter, "-baseline", delimiter, foundationBaseLineStringBuilder.toString(),
				delimiter);
	...
	public static void createRmactivityBatFile(final String viewPath, final String pvob, final String batFilePath) {
		...
		logger.finest(scer.getExecutorResponse().toString());
	...
	public static String getActivitySetInCurrentView(final String viewPath) {
		...
		String[] params = { ClearCaseUtils.getClearToolPath(), "lsactivity", "-cact", "-fmt", "%n" };
		...
		if (scer.getReturnCode() == 0 && scer.getStderr().length() == 0) {
			activity = scer.getStdout().toString();
		}
		return activity;
	}

	public static final String[] getChangeSet(final String activity, final File currentViewFile) {
		String[] clearToolparams = { getClearToolPath(), "lsactivity", "-fmt", "%[versions]Cp",
				"activity:" + activity };
		List<String> parametersList = new ArrayList<String>(Arrays.asList(clearToolparams));
		SystemCommandExecutorResponse systemCommandExecutorResponse = SystemCommandExecutorRunner.run(parametersList,
				currentViewFile);
		...
		if (systemCommandExecutorResponse.getReturnCode() != 0
			...
			logger.warning(systemCommandExecutorResponse.getStderr().toString());
		} else {
	...
	public static String[] getClearLicenseArray() {
		SystemCommandExecutorResponse systemCommandExecutorResponse = SystemCommandExecutorRunner
				.run(ClearCaseUtils.getClearLicensePath(), null);
	...
	public static String[] getComponentsNamesInStream(final String stream) {
		...
		if (componentsSelectors != null) {
			...
			for (int i = 0; i < componentsNames.length; i++) {
				componentsNames[i] = componentsSelectors[i].substring(0,
						componentsSelectors[i].indexOf(OzConstants.AT_SIGN));
			}
		}
	...
	public static List<String> getStreamComponents(final String streamSelector, final String currentDir,
			final String... dynamicViewTag) {
		String viewTag = null;
		if (dynamicViewTag != null && dynamicViewTag.length > 0) {
			viewTag = dynamicViewTag[0];
		}
		if (viewTag != null) {
			List<String> parameterList = ListUtils.getItemsAsList(ClearCaseUtils.getClearToolPath(), "startview",
					viewTag);
			SystemCommandExecutorResponse systemCommandExecutorResponse = SystemCommandExecutorRunner.run(parameterList,
					new File(currentDir));
		}
		List<String> parameterList = ListUtils.getItemsAsList(ClearCaseUtils.getClearToolPath(), "lsstream", "-fmt",
				"\"%[components]CXp\"", streamSelector);
		SystemCommandExecutorResponse systemCommandExecutorResponse = SystemCommandExecutorRunner.run(parameterList,
				new File(currentDir));
	...
	public static List<String> getStreamLatestBls(final String streamSelector, final String currentDir,
			final String... dynamicViewTag) {
		String viewTag = null;
		if (dynamicViewTag != null && dynamicViewTag.length > 0) {
			viewTag = dynamicViewTag[0];
	...
	public static final String getStreamPvob(final String myStream) {
		...
		for (int i = 0; i < pVobs.length; i++) {
			...
			SystemCommandExecutorResponse systemCommandExecutorResponse = ClearCaseUtils
					.runClearToolCommand(clearToolparams);
			if (systemCommandExecutorResponse.getReturnCode() == 0
					&& systemCommandExecutorResponse.getStdout().length() > 0
					&& systemCommandExecutorResponse.getStderr().length() == 0) {
				pvobCounter++;
				pvobIndex = i;
			}
			logger.fine(systemCommandExecutorResponse.getExecutorResponse().toString());
		}
	...
	public static void getUserUsageStats(final String[] clearLicenseArray,
			final Map<String, Integer> clearCaseUserUsageStatsMap,
	...
	private static Properties getViewProperties(final SystemCommandExecutorResponse scer, final String viewIdentifier) {
		...
		if (scer.getReturnCode() == 0 && scer.getStdout() != null && scer.getStdout().length() > 0) {
			...
			PropertiesUtils.printProperties(viewProperties, Level.FINEST);
		} else {
	...
	public static Properties getViewPropertiesByViewRoot(final String viewPath) {
		List<String> params = Arrays.asList(ClearCaseUtils.getClearToolPath(), "lsview", "-l", "-cview");
		SystemCommandExecutorResponse scer = ClearCaseUtils.runClearToolCommand(params, new File(viewPath));
		return getViewProperties(scer, viewPath);
	}

	public static Properties getViewPropertiesByViewTag(final String viewTag) {
		List<String> params = Arrays.asList(ClearCaseUtils.getClearToolPath(), "lsview", "-l", viewTag);
	...
	public static SystemCommandExecutorResponse mkactivity(final List<String> clearToolParametersList,
			final File... directory) {
		/*
		 * mkact/ivity [ –c/omment comment | –cfi/le pname | –cq/uery | –cqe/ach
		 * | –nc/omment ] [ –hea/dline headline ] [ –in stream-selector ] [
		 * –nse/t ] [ –f/orce ] [ activity-selector ...]
		 */
		clearToolParametersList.add(0, "mkactivity");
		clearToolParametersList.add(0, getClearToolPath());
		ListUtils.getAsDelimitedString(clearToolParametersList, Level.INFO);
		SystemCommandExecutorResponse systemCommandExecutorResponse = SystemCommandExecutorRunner
				.run(clearToolParametersList, directory);
		logger.info(systemCommandExecutorResponse.getExecutorResponse().toString());
		return systemCommandExecutorResponse;
	}

	public static SystemCommandExecutorResponse mkactivity(final String[] clearToolParameters,
			final File... directory) {
	...
	public static final boolean removeChangeSet(final String activity, final String logFilePath,
		...
		String[] clearToolparams = new String[] { getClearToolPath(), "lsactivity", "-fmt", "%[versions]Cp",
				"activity:" + activity };
		...
		if (systemCommandExecutorResponse.getReturnCode() == 0) {
			...
			if (changeSetArray != null && changeSetArray.length > 0) {
				...
				clearToolparams = new String[] { "", "rmver", "-force", "-xbranch", "-xlabel", "-xattr", "-xhlink",
						"-nc", "" };
			...
			logger.info("removeChangeSet set for activity " + activity + " completed. return code: "
	...
	public static SystemCommandExecutorResponse runClearToolCommand(final List<String> clearToolParametersList,
			final File... directory) {
		SystemCommandExecutorResponse systemCommandExecutorResponse = SystemCommandExecutorRunner
				.run(clearToolParametersList, directory);
		logger.finest(systemCommandExecutorResponse.getExecutorResponse().toString());
		return systemCommandExecutorResponse;
	}

	public static SystemCommandExecutorResponse runClearToolCommand(final String[] clearToolParametersArray,
			final File... directory) {
	...
	public static SystemCommandExecutorResponse setactivity(final List<String> clearToolParametersList,
			final File[] files) {
		/*
		 * setact/ivity [ –c/omment comment | –cfi/le pname | –cq/uery |
		 * –nc/omment ] [ –vie/w view-tag ] { –none | activity-selector }
		 */
		clearToolParametersList.add(0, "setactivity");
		clearToolParametersList.add(0, getClearToolPath());
		ListUtils.getAsDelimitedString(clearToolParametersList, Level.INFO);
		SystemCommandExecutorResponse systemCommandExecutorResponse = SystemCommandExecutorRunner
				.run(clearToolParametersList, files);
		logger.info(systemCommandExecutorResponse.getExecutorResponse().toString());
		return systemCommandExecutorResponse;
	}
	...
	public static void setClearCaseEnvironmentVariables() {
		...
		if (ccGroups.length > 0) {
			...
			StringBuilder sb = new StringBuilder(
					StringUtils.concat(userDomain, OzConstants.BACK_SLASH, clearCaseUsers));
	...
	public static void setView(final String viewTag) {
		String[] parametersArray1 = { getClearToolPath(), SET_VIEW, viewTag };
		SystemCommandExecutorResponse systemCommandExecutorResponse1 = SystemCommandExecutorRunner
				.run(parametersArray1);
		logger.info(systemCommandExecutorResponse1.getExecutorResponse().toString());
	}

	public static SystemCommandExecutorResponse startView(final String viewTag, final String ccViewRootFolderPath,
			final Boolean... waitParam) {
		Boolean wait = VarArgsUtils.getMyArg(new Boolean(false), waitParam);
		SystemCommandExecutorResponse scer = null;
		if (viewTag != null && viewTag.length() > 0) {
			String[] parametersArray1 = { getClearToolPath(), START_VIEW, viewTag };
			scer = SystemCommandExecutorRunner.run(parametersArray1);
			logger.info("startView " + viewTag + " outcome: " + scer.getExecutorResponse().toString());
			if (scer.getReturnCode() == OzConstants.EXIT_STATUS_OK && wait && ccViewRootFolderPath != null) {
				...
				final int startViewLoopControl = OzConstants.INT_10;
				...
				for (int i = 0; i < startViewLoopControl; i++) {
					if (ccViewRootFolder.isDirectory()) {
						...
						logger.info("waiting for targer folder to become availabe. Iteration: " + String.valueOf(i));
