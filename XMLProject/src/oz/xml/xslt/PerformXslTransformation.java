...
public class PerformXslTransformation {
	...
	private static String getXmlString(final String xmlFilePath) {
		...
		if (xmlFilePath.toLowerCase().startsWith(OzConstants.HTTP.toLowerCase())) {
			xmlString = HTTPUtils.getPageContents(xmlFilePath, XslTransformationParameters
					.getUsername(), XslTransformationParameters.getPassword());
		} else {
	...
	private static void doTransformation(final File xslFile, final String xmlString) {
		if (xslFile.exists()) {
			...
			String output = XsltUtils.transform(xmlString, xslString);
			System.out.print(SystemUtils.LINE_SEPARATOR + output);
			logger.finest("****** transformation using " + xslFile.getAbsolutePath()
					+ " is done *******");
		} else {
