 ...
 * <strong>
 * 
 * NOTE that the ssh1 protocol is a deprecated protocol with known security
 * vulnerabilities. Use version 2 if at all possible.
 * 
 * </strong> Moreover the SSH1 api in MindTerm is less structured and harder to
 * use. That said this class contains code demonstrating:
 * <ul>
 * <li>How to set up a port-forward with SSH1</li>
 * <li>How to start a command over an SSH1 connection and retrieve its output.</li>
 * <li>How to start a shell over an SSH1 connection and run commands in it and
 * retrieve the output</li>
 * <li>How to copy a file with SCP over SSH1</li>
 * </ul>
 * Usage: <code> java -cp examples.jar examples.SSH1Example
...
public class SSH1Example extends SSHInteractorAdapter implements SSHAuthenticator, SSHClientUser {
	...
	/**
	 * This subclass of SSHClient is needed to be able to see which encryption
	...
	class MYSSHClient extends SSHClient {
		 ...
		 * 
		 * @param c
		 *            Cipher algorithm <code>SSH.CIPHER_</code><em>name</em>
		 */
	...
	/**
	 * Create an SSH1Example instance and store the login details in properties.
	 */
	...
	/**
	 * Start an SSH1 connection with port forward. Then demonstrate reading and
	 * writing on this portforward.
	 */
	public void runWithPortFwd() throws UnknownHostException, IOException {
		/*
		 * Add the port forward from localhost:4083 to "a_WebHost" (contant
		 * defined above) port 80. The portforward is of type "general" (=no
		 * plugin)
		 */
	...
	/**
	 * Start a command over SSH1 connection and run a command and retrieve the
	 * output
	 * 
	 * @param command
	 *            Command to execute
	...
	public void runWithCommand(String command) throws IOException {
		...
		SSHConsoleClient client = new SSHConsoleClient(getSrvHost(), getSrvPort(), this, this);
		...
		// Show output
		System.out.println(">>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>");
		BufferedReader in = new BufferedReader(new InputStreamReader(client.getStdOut()));
		for (;;) {
			String line = in.readLine();
			if (line == null)
				break;
	...
	/**
	 * Start a shell over SSH1 connection and run a command and retrieve the
	 * output
	 * 
	 * @param command
	 *            Command to execute
	...
	private static int[] ciphers = { SSH.CIPHER_BLOWFISH, SSH.CIPHER_IDEA, SSH.CIPHER_3DES,
			SSH.CIPHER_RC4, SSH.CIPHER_DES };
	...
	/**
	 * Return a connection to the server. This can be used to connect through
	 * proxies etc.
	 */
	...
	public static void main(String argv[]) {
		...
		try {
			SSH1Example ssh1 = new SSH1Example(argv[0], argv[1], argv[2], argv[3]);
			ssh1.runWithPortFwd();
