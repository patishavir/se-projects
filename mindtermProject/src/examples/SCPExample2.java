...
/**
 * Copy a file to/from an SSH1 or SSH2 server using the scp protocol. The scp
 * protocol was introduced with ssh1 and is very similar to the rcp protocol.
 * <p>
 * See the <code>main</code> method for an example on how to do a file transfer,
 * and also how to expand glob expressions.
 * <p>
 * The <code>copyFilesWithSSH1</code> method will copy files using the SSH1
 * protocol.
 * <p>
 * The <code>copyFilesWithSSH2</code> method will copy files using the SSH2
 * protocol.
 * <p>
 * The <code>doGlob</code> method will expand glob expressions.
 * <p>
...
public class SCPExample2 {
	...
	private static String findPrivateKeyFile(int version) {
		String base = System.getProperty("user.home") + File.separatorChar + ".ssh"
				+ File.separatorChar;
		if (version == 1)
	 ...
	 *            the private key file, if null we will try to find the OpenSSH
	 *            private key file
	 * @param privatekeypassword
	 *            the password for the private key file, null if none needed
	 * @param files
	 *            an array of files that should be transferred
	 * @param destination
	 *            the destination file or directory
	 * @param toremote
	 *            true if files should be copied to the remote server, otherwise
	 *            false
	 */

	public static void copyFilesWithSSH1(String server, int port, String user,
			String privatekeyfile, String privatekeypassword, String[] files, String destination,
			boolean toremote) throws IOException, UnknownHostException {
		...
		try {
			...
			MySSHInteractor interactor = new MySSHInteractor(user, privatekeyfile,
					privatekeypassword);
	...
	public static void copyFilesWithSSH2(String server, int port, String user,
			String privatekeyfile, String privatekeypassword, String[] files, String destination,
			boolean toremote) throws IOException, UnknownHostException, SSH2Exception {
		...
		try {
			...
			// authentication
			transport = new SSH2Transport(new Socket(server, port), createSecureRandom());

			if (privatekeyfile == null)
				privatekeyfile = findPrivateKeyFile(2);

			SSH2SimpleClient client = new SSH2SimpleClient(transport, user, privatekeyfile,
					privatekeypassword);

			// Create SSH2SCP1 client which is used for the file transfer
			SSH2SCP1Client scpClient = new SSH2SCP1Client(new File(System.getProperty("user.dir")),
	...
	public static void main(String[] argv) {
		if (argv.length < 6) {
			System.out
					.println("Usage: SCPExample2 <server:port> <username> 1|2 to|from <srcfiles...> <destination>");
			System.exit(1);
		}
		...
		try {
			...
			if (argv[2].equals("1")) {
				copyFilesWithSSH1(server, port, user, null, null,
						(String[]) v.toArray(new String[v.size()]), argv[argv.length - 1],
						argv[3].equals("to"));
			} else {
