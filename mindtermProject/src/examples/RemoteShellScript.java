...
/**
 * This is a simple demo of running a list of command-lines given as text file.
 * The commands are run sequentially and the stdout output from each command is
 * printed to the local stdout (stderr is also redirected to the local stderr).
 * <p>
 * Usage: <code> java -cp examples.jar examples.RemoteShellScript
...
public class RemoteShellScript implements ExpectOutputStream.Expector {
	...
	/**
	 * Constructor which takes the remote console where the script should be
	 * executed as argument.
	 * 
	 * @param console
	...
	public String run(String cmd) {
		try {
			...
			/*
			 * Send end-marker (which the ExpectOutputStream is waiting for).
			 * The trick with the quotes is so that the ExpectOutputStream does
			 * not trigger when the echo command is echoed but just when it is
			 * executed.
			 */
	...
	public static void main(String[] argv) {
		if (argv.length < 4) {
			System.out
					.println("usage: RemoteShellScript <server[:port]> <username> <password> <script-file>");
			System.exit(1);
		}
		...
		try {
			...
			/*
			 * How to set some preferences, i.e. force usage of blowfish cipher
			 * and set loglevel to 6 (debug) and log output to the file
			 * ssh2out.log
			 * 
			 ...
			 */
			SSH2ConsoleRemote console = new SSH2ConsoleRemote(client.getConnection(), null,
					System.err);
			...
			BufferedReader script = new BufferedReader(new InputStreamReader(new FileInputStream(
					file)));
	 ...
	 */
	public synchronized void reached(ExpectOutputStream out, byte[] buf, int len) {
