 ...
 * <p>
 * Usage: <code> java -cp examples.jar examples.InteractRemoteCommand
...
public class InteractRemoteCommand {
	...
	public static void main(String[] argv) {
		if (argv.length < 4) {
			System.out
					.println("usage: InteractRemoteCommand <server[:port]> <username> <password> <command-line>");
			System.exit(1);
		}
		try {
			/*
			 ...
			 * it here). The final boolean value indicates if the remote end
			 * should create a real pty or not. A pty is sometimes needed to
			 * catch password queries etc.
			 */
			...
			/*
			 * Wait for output. This is just sample code so it is hardcoded to
			 * wait for a passwrod prompt.
			 * 
			 * It is also possible to use com.mindbright.util.ExpectOutputStream
			 * to achieve this. See examples.RemoteShellScript for an example.
			 */
			String data;
			char c;
