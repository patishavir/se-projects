...
/**
 * This is a simple demo of running a single command on a ssh2 server in exactly
 * the same way an ordinary unix ssh2 client works when passed a command-line to
 * be run.
 * <p>
 * Usage: <code> java -cp examples.jar examples.RunRemoteCommand
...
public class RunRemoteCommand {
	...
	public static void main(String[] argv) {
		if (argv.length < 4) {
			System.out
					.println("usage: RunRemoteCommand <server[:port]> <username> <password> <command-line>");
			System.exit(1);
		}
		try {
			 ...
			 * authentication (if other authentication methods are needed check
			 ...
			 * command execution to our own stdout and stderr for simplicity.
			 */
