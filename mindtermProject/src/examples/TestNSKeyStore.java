 ...
 * <p>
 * Usage: <code> java -cp examples.jar examples.TestNSKeyStore
...
public class TestNSKeyStore {
	...
	public static void main(String[] argv) {
		try {
			// Check arguments
			if (argv.length != 2) {
				System.out.println("usage: TestNSKeyStore <dirname> " + "<password>");
				System.exit(1);
			}
			...
			ByteArrayInputStream ba = new ByteArrayInputStream(dirName.getBytes());
			...
			while (e.hasMoreElements()) {
				...
				System.out.println(alias + " (" + (isKey ? "cert" : "trusted cert") + ")");
				System.out.println("\t" + ns.getCertificate(alias));
				if (isKey) {
					Key key = ns.getKey(alias, password.toCharArray());
					if (key instanceof com.mindbright.jca.security.interfaces.RSAPrivateKey) {
