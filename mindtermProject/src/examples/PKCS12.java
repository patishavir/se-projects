 ...
 * <p>
 * Usage: <code> java -cp examples.jar examples.PKCS12
 * <em>filename</em> <em>password</em>
 */
public final class PKCS12 {

	private static byte[] deriveKey(String password, int keyLen, byte[] salt, int iterations,
			int id, String digAlg) {
		try {
	...
	private static void doCipher(int mode, String password, byte[] input, int len, byte[] output,
			byte[] salt, int iterations, String ciperType) throws Exception {
		try {
			Cipher cipher = Cipher.getInstance(ciperType);
			int keyLen = (ciperType.startsWith("3DES") ? (192 / 8) : (40 / 8));
			byte[] key = deriveKey(password, keyLen, salt, iterations, 1, "SHA");
			byte[] iv = deriveKey(password, 8, salt, iterations, 2, "SHA");
			cipher.init(mode, new SecretKeySpec(key, cipher.getAlgorithm()),
					new IvParameterSpec(iv));
			cipher.doFinal(input, 0, len, output, 0);
		} catch (NoSuchAlgorithmException e) {
			throw new Exception("Invalid algorithm in " + "PKCS12.doCipher: " + e);
		} catch (InvalidKeyException e) {
			throw new Exception("Invalid key derived in " + "PKCS12.doCipher: " + e);
		}
	}
	...
	public static void main(String[] argv) {
		try {
			...
			System.out.println("PFX authSafe type: " + pkcs12.authSafe.contentType.getString());

			checkMac(pkcs12, password);

			AuthenticatedSafe authSafe = new AuthenticatedSafe();
			ASN1OctetString data = (ASN1OctetString) pkcs12.authSafe.content.getValue();
			java.io.ByteArrayInputStream ba = new java.io.ByteArrayInputStream(data.getRaw());
			...
			for (int i = 0; i < authSafe.getCount(); i++) {
				...
				if (cit.equals("1.2.840.113549.1.7.1")) {
					...
					PKCS12PbeParams params = (PKCS12PbeParams) ed.encryptedContentInfo.contentEncryptionAlgorithm.parameters
							.getValue();
					...
					doCipher(Cipher.DECRYPT_MODE, password, enc, enc.length, dec, salt, iterations,
	...
	private static void checkMac(PFX pfx, String password) throws Exception {
	...
	private static void processSafeContents(SafeContents sc, String password) throws Exception {
		...
		for (int j = 0; j < sc.getCount(); j++) {
			...
			System.out.println("SafeBag type (" + j + ") = " + sb.bagId.getString());
			printSafeBagAttr(sb);
			if (sb.bagId.getString().equals("1.2.840.113549.1.12.10.1.2")) {
				EncryptedPrivateKeyInfo kb = (EncryptedPrivateKeyInfo) sb.bagValue.getValue();
				System.out.println("PKCS#8 shrouded key alg = "
						+ kb.encryptionAlgorithm.algorithmName());

				PKCS12PbeParams params = (PKCS12PbeParams) kb.encryptionAlgorithm.parameters
						.getValue();
				...
				for (int i = 0; i < x509cert.tbsCertificate.extensions.getCount(); i++) {
					Extension e = (Extension) x509cert.tbsCertificate.extensions.getComponent(i);
					System.out.println("extension: " + e.extnID.getString() + ", crit = "
							+ e.critical.getValue());
					;
				}
		...
		for (int i = 0; i < cnt; i++) {
			...
			while (true) {
				if (v instanceof ASN1CharString) {
					...
					aValue = com.mindbright.util.HexDump.toString(((ASN1OctetString) v).getRaw());
				}
				break;
			}
