 ...
 */
public final class BasicClient extends WindowAdapter implements SSH2Interactor,
	...
	public void run() {
		try {
			...
			SecureRandomAndPad secureRandom = new SecureRandomAndPad(new SecureRandom(
					seed.getBytesBlocking(20, false)));
		...
		for (i = 0; i < 32; i++) {
			...
			try {
				...
				client.getConnection().newLocalForward((String) components[1],
						((Integer) components[2]).intValue(), (String) components[3],
	...
	public String[] promptMultiFull(String name, String instruction, String[] prompts,
			boolean[] echos) throws SSH2UserCancelException {
		if (prompts.length == 1) {
