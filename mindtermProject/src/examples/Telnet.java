...
/**
 * Open a telnet window and connect with the telnet protocol. SSH is not
 * involved att all.
 * <p>
 * Usage: <code> java -cp examples.jar examples.Telnet
...
public class Telnet extends WindowAdapter {
	...
	/**
	 * Constructor which will cause the window to prompt the user for the server
	 * to connect to.
	 */
	public Telnet() {
		this(null, -1);
	...
	/**
	 * Constructor which will cause a connection to the default port (23) on the
	 * given host.
	 */
	...
	/**
	 * Constructor which will cause a connection to a specified port on the
	 * given host.
	 */
	...
	public void startMeUp() {
		...
		TerminalFrameTitle frameTitle = new TerminalFrameTitle(frame, "Telnet (not connected)");
		...
		try {
			...
			telnetAdapter = new TelnetTerminalAdapter(socket.getInputStream(),
					socket.getOutputStream(), terminal);

			// Update window title to reflect where we are connected
			frameTitle.setTitleName("telnet@" + remoteHost
			...
			/*
			 * Check that the terminal is still in local echo mode If so, print
			 * a message that we have disconnected
			 */
	...
	/**
	 * Parse remote host name. This sets the remote host to connect to. It also
	 * looks if a port-number has been specified via the :<em>portnr</em>
	 * mechanism.
	 */
