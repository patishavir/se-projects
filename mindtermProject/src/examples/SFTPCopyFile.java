...
/**
 * Copy a file to/from an SSH2 server using the sftp protocol. The sftp protocol
 * was introduced with ssh version 2.
 * <p>
 * Usage: <code> java -cp examples.jar examples.SFTPCopyFile
...
public class SFTPCopyFile {
	...
	public static void main(String[] argv) {
		if (argv.length < 6) {
			System.out
					.println("Usage: SFTPCopyFile <server:port> <username> <password> to|from <src_file> <dst_file>");
			System.exit(1);
		}
		try {
			...
			/*
			 * Create SFTP client which is used for the file transfer. This
			 * instance can be used multiple times to transfer many files
			 * although this exampel only uses it once. We use asynchronous mode
			 * for the SSH2SFTPClient beacuse it is much faster.
			 */
			SSH2SFTPClient sftpClient = new SSH2SFTPClient(client.getConnection(), false);
			...
			/*
			 * Copy the given file The code here only copies one file but any
			 * number of operations can be peformed on the SSH2SFTClient
			 */
	 ...
	 * 
	 * @param sftpClient
	 *            connection to copy over
	 * @param srcFile
	 *            local file to copy from
	 * @param dstFile
	 *            remote file to copy to
	 */
	private static void copyTo(SSH2SFTPClient sftpClient, String srcFile, String dstFile)
			throws Exception {
		FileHandle handle = sftpClient.open(dstFile, SSH2SFTP.SSH_FXF_WRITE
				| SSH2SFTP.SSH_FXF_CREAT | SSH2SFTP.SSH_FXF_TRUNC, new SSH2SFTP.FileAttributes());
	 ...
	 *            connection to copy over
	 * @param srcFile
	 *            remote file to copy from
	 * @param dstFile
	 *            local file to copy to
	 */
	private static void copyFrom(SSH2SFTPClient sftpClient, String srcFile, String dstFile)
			throws Exception {
		FileHandle handle = sftpClient.open(srcFile, SSH2SFTP.SSH_FXF_READ,
