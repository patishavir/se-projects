 ...
 * <p>
 * Usage: <code> java -cp examples.jar examples.FTPToSFTPProxy
 * [<em>listen_address</em>[:<em>port</em>]]
 */
public class FTPToSFTPProxy extends SSH2FTPOverSFTP {

	private final static String IDENTITY = "FTPToSFTPProxy use 'ssh2-user@ssh2-host[:port]' as your username.";
	 ...
	 * 
	 * @param ftpInput
	 *            input stream from ftp client
	 * @param ftpOutput
	 *            output stream to ftp client
	 */
	...
	public boolean login(String user, String pass) {
		...
		try {
			...
			transport = new SSH2Transport(ssh2Socket, prefs, createSecureRandom());
	...
	public static void main(String[] argv) {
		...
		try {
			...
			System.out.println("** FTPToSFTPProxy listening on " + server + ":" + port);
			ServerSocket listen = new ServerSocket(port, 32, InetAddress.getByName(server));
			while (true) {
				Socket conn = listen.accept();
				FTPToSFTPProxy proxy = new FTPToSFTPProxy(conn.getInputStream(),
