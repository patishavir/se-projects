...
public class CICSAdapter {
	 ...
	 * Constructor.<br/>
	 * 
	 * Connection string data is defined from the end point used. If no URL is
	 * defined then the default connection string will be used.<br/>
	 * 
	 * @param cicsEndpoint
	...
	public CICSAdapter(CICSEndpoint cicsEndpoint) throws IOException {
		...
		try {
			...
			// The JavaGateway represents a logical connection between your
			// program and the CICS Transaction Gateway
			// when you specify a remote protocol.
			// If you specify a local connection, you connect directly to the
			// CICS server, bypassing any CICS Transaction
			// Gateway servers.
			...
			//
			// When you create a JavaGateway you determine the protocol to use,
			// and if required,
			// the connection details of the remote CICS Transaction Gateway
			// server (network address and port number).
			// With the JavaGateway class you can either specify this
			// information using the setAddress(), setProtocol() and
			// setPort() methods, or you can provide all the information in URL
			// form of Protocol://Address:Port.
			// You can use the setURL() method or pass the URL into one of the
			// JavaGateway constructors.
	...
	public String runTransaction(String programName, String transactionId, String inputCommArea, int commAreaSize)
	...
	public byte[] runTransaction(String programName, String transactionId, byte[] inputCommArea)
			throws IOException, Exception {
		// Shared CommArea in byte format
		...
		try {
			LOGGER.info("New ECIRequest in {} server to call {} program with {} transaction", this.endpoint.getServer(),
					programName, (null != transactionId ? transactionId : "no"));

			// ECI Request to call CICS Transation Gateway
			ECIRequest eciRequest = new ECIRequest(this.endpoint.getServer(), // CICS
																				// Server
			...
			if (LOGGER.isInfoEnabled()) {
				LOGGER.info(
						"Input CommArea Byte[] Data:\n-** INPUT BYTE[] COMMAREA **-\n{}\n-** END INPUT BYTE[] COMMAREA **-",
						inputCommArea);
			}
			...
			if (LOGGER.isInfoEnabled()) {
				LOGGER.info(
						"Output CommArea Byte[] Data:\n-** OUTPUT BYTE[] COMMAREA **-\n{}\n-** END OUTPUT BYTE[] COMMAREA **-",
						byteCommArea);
			}
		} catch (Exception e) {
	 ...
	 * codes are printed to describe the error before the program exits. Note:
	 * Security may be required for client connection to the server and not just
	 * for the ECI request. Refer to the security chapter in the product
	 * documentation for further details.
	 * 
	 * @param requestObject
	 * 
	 * @return <code>true</code> if flow was executed successfully,
	...
	private boolean flowRequest(ECIRequest requestObject) throws IOException {
		...
		try {
			...
			LOGGER.info("CicsCodes: {}-{} on Gateway {}", requestObject.getCicsRc(), requestObject.getCicsRcString(),
					this.gateway);
			LOGGER.info("RcCodes: {}-{} on Gateway {}", requestObject.getRc(), requestObject.getRcString(),
					this.gateway);
			// }
			...
			if (iRc != 0) {
				if (requestObject.getCicsRc() == 0) {
					...
					if (requestObject.getCicsRc() == ECIRequest.ECI_ERR_SECURITY_ERROR
							|| (requestObject.Abend_Code != null
									&& requestObject.Abend_Code.equalsIgnoreCase("AEY7"))) {
