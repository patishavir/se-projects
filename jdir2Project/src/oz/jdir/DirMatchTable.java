...
public final class DirMatchTable implements Runnable {
	...
	private void build2SidedFileInfoPtrs() {
		...
		do {
			...
			logger.finest("sdPath2Compare=" + sdPath2Compare + "   ddPath2Compare="
					+ ddPath2Compare);
			if (sdPath2Compare.equals(ddPath2Compare)) {
				...
				if ((showAll)
						|| (showEqual && FileComparison.compareFiles(sdFileInfo, sdPath2Compare,
								sdFullPath, ddFileInfo, ddPath2Compare, ddFullPath))
						|| (showDiff && !FileComparison.compareFiles(sdFileInfo, sdPath2Compare,
								sdFullPath, ddFileInfo, ddPath2Compare, ddFullPath))) {
		...
		logger.info("Completed buidMatchTable processing in " + myStopWatch.getElapsedTimeString()
				+ " using " + JdirParameters.getFileComparisonCriteria());
