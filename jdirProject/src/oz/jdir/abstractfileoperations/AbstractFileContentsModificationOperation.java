...
import oz.jdir.fileoperations.ClearCaseOperation;
...
public abstract class AbstractFileContentsModificationOperation extends AbstractFileOperation {
	...
	private boolean processFile(final JdirInfo sd, final JdirInfo dd, final int jdirIndex,
		...
		if (validateFilePath(currentFilePath, currentFile)) {
			...
			ClearCaseAttributes clearCaseAttributes = currentFileInfo.getClearCaseAttributes();
			boolean checkoutReturnCode = false;

			boolean updateAndCheckoutRequired = !currentFile.canWrite()
					&& JdirParameters.isClearCaseIntegrationEnabled()
					&& clearCaseAttributes != null && clearCaseAttributes.isCCmaganaged()
					&& !clearCaseAttributes.isCheckedOut();
			if (updateAndCheckoutRequired) {
				boolean updateReturnCode = new ClearCaseOperation().exec(sd, dd, myJdirIndex,
						fileOperationJDialog, FileOperationsEnum.Update);
				if (!updateReturnCode) {
					logger.warning(" Update failed for " + currentFilePath
				...
				logger.finest(" "
						+ currentFilePath
						+ " last modified: "
						+ DateTimeUtils.formatDate(currentFile.lastModified(),
								"dd/MM/yyyy HH:mm:ss"));
			}
			...
			if (validateFileContents(currentFileContents, currentFilePath)) {
				...
				if (!newFileContents.equals(currentFileContents)) {
					if (updateAndCheckoutRequired) {
						checkoutReturnCode = new ClearCaseOperation().exec(sd, dd, myJdirIndex,
								fileOperationJDialog, FileOperationsEnum.Checkout);
						if (!checkoutReturnCode) {
							logger.warning(" Checkout failed for " + currentFilePath
									+ " file has not been processed!");
							return false;
						}
					}
					...
					if (checkoutReturnCode && fileWritten) {
						new ClearCaseOperation().exec(sd, dd, myJdirIndex, fileOperationJDialog,
								FileOperationsEnum.Checkin);
					}
					if (checkoutReturnCode && !fileWritten) {
						new ClearCaseOperation().exec(sd, dd, myJdirIndex, fileOperationJDialog,
								FileOperationsEnum.Undocheckout);
					}
