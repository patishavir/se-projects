...
public enum FileOperationsEnum {
	...
	CopyAll("Copy all files from source to destination directory", "CopyOperation",
			...
			FileOperationTypesEnum.selectedFilesOperation, 2), AddResponsibleTeamComment(
			"Add responsible team comment to selected files", "AddResponsibleTeamCommentOperation",
			FileOperationTypesEnum.selectedFilesOperation, 1),
	//
	add2SourceControl("Add selected files to source control", "MkelemOperation",
			FileOperationTypesEnum.clearCaseOperation, 1), Checkout("Checkout selected files",
			"ClearCaseOperation", FileOperationTypesEnum.clearCaseOperation, 1), Undocheckout(
			"Undo checkout selected files", "ClearCaseOperation",
			FileOperationTypesEnum.clearCaseOperation, 1), Checkin("Checkin selected files",
			"ClearCaseOperation", FileOperationTypesEnum.clearCaseOperation, 1), RecursiveCheckin(
			"Recursive Checkin files", "RecursiveCheckinOperation",
			FileOperationTypesEnum.clearCaseOperation, 1), RecursiveAdd2SourceControl(
			"Recursive add 2 source control", "RecursiveAdd2SCOperation",
			FileOperationTypesEnum.clearCaseOperation, 1), Update("Update", "ClearCaseOperation",
			FileOperationTypesEnum.clearCaseOperation, 1), Versiontree("Version tree",
			"ClearCaseOperation", FileOperationTypesEnum.clearCaseOperation, 1), RebasePrep(
			"RebasePrep", "RebasePrepOperation", FileOperationTypesEnum.clearCaseTwoSidedOperation,
			2);
