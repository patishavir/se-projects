...
public class CopyOperation extends AbstractFileOperation {
	...
	public final boolean exec(final JdirInfo sd, final JdirInfo dd, final int jdirIndex,
		...
		try {
			...
			/*
			 * do ClearCase processing
			 */
			boolean ccCopyReturnCode = true;
			boolean copyRC = false;
			if (JdirParameters.isClearCaseIntegrationEnabled()) {
				ClearCaseClass ccc = new ClearCaseClass();
				if (destinationFile.exists()
						&& destinationFileInfo.getClearCaseAttributes() != null
						&& destinationFileInfo.getClearCaseAttributes().isCCmaganaged()) {
					ccCopyReturnCode = new ClearCaseOperation().exec(dd, null, jdirIndex,
							fileOperationJDialog, FileOperationsEnum.Checkout);
				}
				if (ccc.isElementManaged(sourceFilePath)
						&& ccc.isElementManaged(existingoutputPart)
						&& !existingoutputPart.equalsIgnoreCase(destinationFilePath)) {
					logger.fine("Copy1 mkelem parameter=" + existingoutputPart
							+ " mkelem parameter 2="
							+ destinationFilePath.substring(existingoutputPart.length()));
					ccCopyReturnCode = ccc.mkelem(existingoutputPart,
							destinationFilePath.substring(existingoutputPart.length()));
				}
				if (!ccCopyReturnCode) {
					return ccCopyReturnCode;
				}
			}
			//
