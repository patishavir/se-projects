...
public class JdirToolBar extends JToolBar {
	...
	JdirToolBar(final ActionListener dirFrameActionListener) {
		...
		if (JdirParameters.isClearCaseSupported()) {
			this.add(Box.createRigidArea(rigidAreaDimension));
			rebasePrepButton = new TBButton("RebasePrep", ORANGE_COLOR,
					"Perform preparations for ClearCase rebase operation", buttonDimension, this,
					dirFrameActionListener);
			rebasePrepButton.setActionCommand(FileOperationsEnum.RebasePrep.toString());
			FileOperationsEnum.RebasePrep.setJButton(rebasePrepButton);
			if (JdirParameters.isClearCaseIntegrationEnabled()) {
				rebasePrepButton.setVisible(true);
			} else {
				rebasePrepButton.setVisible(false);
			}
		}
		this.add(Box.createRigidArea(rigidAreaDimension));
		...
		if (JdirParameters.isClearCaseSupported()) {
			String ccText;
			if (JdirParameters.isClearCaseIntegrationEnabled()) {
				ccText = ClearCaseIntegration.getDISABLECC_TEXT();
			} else {
				ccText = ClearCaseIntegration.getENABLECC_TEXT();
			}
			CCButton = new TBButton(ccText, Color.red, "Start ClearCase integration",
					buttonDimension, this, dirFrameActionListener);
			CCButton.setActionCommand(GuiComponentsEnum.EnableCC.toString());
		}
		//
		this.add(Box.createRigidArea(rigidAreaDimension));
