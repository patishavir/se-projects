...
public class JdirMenuBar extends JMenuBar {
	...
	private static JMenuItem jmiEnableCC;
	...
	JdirMenuBar(final ActionListener jdirActionlistenerP) {
		...
		if (JdirParameters.isClearCaseSupported()) {
			if (JdirParameters.getNumberOfPanes() > 1) {
				jmiRebasePrep = new JMenuItem("RebasePrep");
				jmiRebasePrep.setToolTipText("Perform preparations for ClearCase rebase operation");
				jmFile.add(jmiRebasePrep);
				setActionCommandaddActionListener(jmiRebasePrep,
						FileOperationsEnum.RebasePrep.toString());
				FileOperationsEnum.RebasePrep.setJMenuItem(jmiRebasePrep);
			}
			jmiEnableCC = new JMenuItem(ClearCaseIntegration.getENABLECC_TEXT());
			jmiEnableCC.setToolTipText("Start ClearCase integration");
			jmFile.add(jmiEnableCC);
			setActionCommandaddActionListener(jmiEnableCC, GuiComponentsEnum.EnableCC.toString());
			jmFile.addSeparator();
		}
		jmFile.add(jmiExit);
		//
	...
	public static JMenuItem getJmiEnableCC() {
		return jmiEnableCC;
	}
